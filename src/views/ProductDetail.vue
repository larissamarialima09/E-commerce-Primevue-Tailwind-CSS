<template>
  <main class="p-6 lg:p-12 bg-black min-h-screen">
    ...
    <div v-else class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
      <div
        class="bg-[#fafafa] rounded-3xl p-0 aspect-square flex items-center justify-center border border-zinc-800 shadow-2xl shadow-emerald-500/5 overflow-hidden"
      >
        <img
          :src="product.image"
          :alt="product.name"
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="flex items-center gap-4">
          <span
            class="bg-emerald-500/10 text-emerald-500 px-3 py-1 rounded-full text-sm font-bold border border-emerald-500/20"
          >
            Em Estoque
          </span>
          <span class="text-zinc-500 font-mono text-xs uppercase tracking-tighter"
            >SKU: PROD-00{{ product.id }}</span
          >
        </div>
        ...
        <div class="flex flex-col gap-1">
          ...
          <span class="text-emerald-500 text-sm font-medium"
            >ou 10x de R$ {{ (product.price / 10).toFixed(2).replace('.', ',') }} sem juros</span
          >
        </div>
